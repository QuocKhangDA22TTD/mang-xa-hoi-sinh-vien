...
const uploadImage = async (file) => {
  ...
  return res.data.url; // URL công khai trả về
};
...
function CreateAPersonalProfile() {
  ...
  const handleCreateProfile = async () => {
    ...
    try {
      const existing = await axios.get(`/api/profile/${getUserIdFromToken()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      try {
        ...
        setMessage(err.response?.data?.message || 'Tạo hồ sơ thất bại');
      }
    }
  };
